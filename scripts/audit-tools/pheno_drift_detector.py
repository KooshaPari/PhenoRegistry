...
"""
pheno-drift-detector: App-substrate drift detector (companion to pheno-predict
and pheno-framework-lint; part of the v8 weekly heavy-runner cron bundle).

Scans PAUSED / CONDITIONAL / CAPSTONE app repos for 2+ non-trivial capabilities
that match the substrate pattern (per ADR-023 Rule 3). Drift hits are output
as GitHub-issue-ready JSON / Markdown for weekly cron → issue auto-creation.

Run weekly via cron (see ops/heavy-runner-cron/INSTALL.md).

Usage:
    pheno-drift-detector scan --root .. --out hits.json
    pheno-drift-detector scan --root .. --format md
...
- A "non-trivial capability" is heuristic: a directory under the app repo with
  ≥ 3 source files (per MIN_DIR_FILES), ≥ 5 KB total (per MIN_DIR_BYTES),
  at least one file matching a Port trait pattern (`trait Foo {`, `interface
  Foo {`, `protocol Foo`, `impl X for Y`, etc.).
...
- Drift score = 1.0·n + 0.4·n_ports + 0.3·n_adapters + 0.3·n_tests, with
  threshold DRIFT_THRESHOLD (1.5). Above the threshold we emit a hit.
- The 4-criterion candidate profile (≥3 files, ≥5 KB, ≥1 port, ≥1 adapter)
  comes from ADR-023 Rule 3 substrate placement.
"""
...
# ---------------------------------------------------------------------------
# Drift scoring (per ADR-023 Rule 3 substrate placement)
# ---------------------------------------------------------------------------
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    # Manual confirmation expected (HITL gate per ADR-043 §5)
    if args.yes:
