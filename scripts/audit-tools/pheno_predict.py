...
"""
pheno-predict: Fleet-wide similar-code scanner for predictive DRY (ADR-041).
...
This is ADR-041 / L72 implementation. Schema is documented in
...
# ---------------------------------------------------------------------------
# ADR-041 4-criteria check
# ---------------------------------------------------------------------------

def check_4_criteria(c: Candidate) -> Candidate:
    """Heuristic check of the 4 ADR-041 predictive-DRY criteria.
...
def render_md(cands: list[Candidate]) -> str:
    ...
    out.append("")
    out.append("### Criterion notes (per ADR-041)")
...
def cmd_check_criteria(args: argparse.Namespace) -> int:
    """Check the 4 ADR-041 criteria for a single candidate (JSON in, JSON out)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    check = sub.add_parser("check-criteria", help="run 4 ADR-041 criteria check on a candidate")
