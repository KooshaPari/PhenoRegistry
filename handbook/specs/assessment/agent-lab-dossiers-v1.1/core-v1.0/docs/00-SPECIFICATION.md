# Product Evaluation Protocol — v1.0 final handoff

**Release:** 1.0.0 · **Prepared for:** the agent-operated software lab · **Date:** September 10, 2026 (America/Los_Angeles).

**Status:** final consolidated specification and operating kit for this request. Adoption of its policies remains subject to actual authority. The package includes an original candidate catalog and an executable reference kernel; it is not a completed deployment of AgilePlus, Tracera or an autonomous lab, and it is not a regulatory certification.

This document incorporates both prior versions. If a retained historical statement conflicts with current v1 operating instructions, the v1 instructions and explicit release limitations govern this package. Prior files are retained unchanged under `provenance/history/`, not used as additional live authorities.

## Release invariants

Agents may originate intent, repository boundaries, requirements, tests, graders and lifecycle decisions inside a real standing mandate. Human authorship is not a prerequisite. Authority, factual support and commitment are separate.

The lab must operate through existing harnesses and subject-held files without waiting for central platforms. An agent doing the work “by hand” follows this protocol with existing tools; a person need not fill out the records.

A catalog entry is not a qualified check. A qualified check is not a product result. A digest-valid result is not authenticated truth. A product pass is not beneficiary value or permission to publish.

Freeze acceptance meaning during each execution epoch. Preserve agent learning and failures across restarts. Classify changes to product, evidence, scope, evaluator and policy separately. Permit corrections and strategic experiments without turning ambiguous or unsupported ideas into mandatory debt.

## Reading path

Start with the root `START-HERE-AGENT.md`. Use `06-EXECUTION-SOP.md` for the working loop, `05-ASSIGNMENT-QUALIFICATION.md` for grader construction, and `12-REFERENCE-KERNEL.md` for executable commands. Schemas, templates, profiles and evidence examples are linked from the root README. Sections below provide the integrated conceptual specification.
...
### Historical source findings and limits

The v0.1/v0.2 source inspections recorded the following. They are retained historical findings, not a new current repository audit; none establishes complete local-device or Git-history coverage:
...
Those earlier inspections did not build, test, modify, merge, deploy or archive the portfolio repositories. The v1.0 package tests its own reference kernel and synthetic toy only; no device-local portfolio audit was performed.
...
Shared schemas belong in the verified shared-contract owner; do not create a new repository merely to hold this protocol. The immediate implementation is a portable agent procedure, subject-held files, existing machine checks, and small adapters only where needed. A CLI, database, graph service, or live AgilePlus/Tracera deployment is not required for bootstrap operation. Every owner/approver role in this table may be an authorized agent function; the role does not imply a human bottleneck.
...
A useful source-drift example: The earlier capture found an eight-pillar product-page description and a nine-pillar documentation description; the v1 handoff does not treat that historical discrepancy as a newly confirmed current discrepancy. Preserve the source versions instead of treating the label “Factory readiness” as an immutable definition. [E1-E2]
...
The bootstrap acceptance test is not “a large catalog exists.” It is: one real failing outcome is detected without a human interpreting undocumented context; it becomes a correctly owned, bounded action; a repair or experiment runs within authority; an independent verifier demonstrates the result; the subject records reconcile and registry delivery is either receipted or honestly queued/blocked; a planted false pass is rejected; and no unrelated maturity claim is promoted.
...
The protocol may initially operate with existing audit folders, one assignment capsule, per-attempt records, and a current checkpoint. The templates are portable proposed records, not a required new folder hierarchy or service. The shipped reference kernel validates selected structures and reductions; it does not implement the entire lab.
...
Subsequent expansion can cover the full catalog and stronger automation. Completion of these documents is not completion of the loop. The root `START-HERE-AGENT.md` is the immediate session handoff; `templates/*.template.json` and `tools/pep.py init-assessment` produce unaccepted starting records. The reference validator checks shape and consistency, not authorization or substantive truth.
...
New primary sources consulted for bootstrap operation.2 on 2026-09-10:
