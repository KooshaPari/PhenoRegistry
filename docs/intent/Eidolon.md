---
repo: "Eidolon"
aliases: []
role: unknown
status: active
last_verified: 2026-06-17
bound_prompts: 4
bound_plans: 0
bound_responses: 0
device: macbook
---

# Intent — Eidolon

## Intent Statement

&lt;To be filled in by hand from the most recent binding prompt. This repo is bound to 4 prompts, 0 plans, and 0 agent responses captured between 2025-08 and 2026-06-17.&gt;

## Bound Prompts

| Date | Source | File | Tag |
| ---- | ------ | ---- | --- |
| ? | claude-code | `docs/curated-prompts/claude-code/unknown/a5bb05aa44bb6eea.md` | policy-setting |
| ? | claude-code | `docs/curated-prompts/claude-code/unknown/5f31d868d6616604.md` | narrative |
| ? | claude-code | `docs/curated-prompts/claude-code/unknown/4fef3f59b18fc20e.md` | policy-setting |
| ? | claude-code | `docs/curated-prompts/claude-code/unknown/30f5f9a3335d065f.md` | narrative |

## Bound Plans

| Date | Source | File | Status |
| ---- | ------ | ---- | ------ |

## Bound Responses (specs, ideas, plans from agents)

| Date | Source | File | Kind |
| ---- | ------ | ---- | ---- |

## Boundary

See: [`docs/boundary/Eidolon.md`](../boundary/Eidolon.md)

## Ecosystem Role

&lt;See                    for the canonical ecosystem role.&gt;

## Open Questions

- &lt;To be filled from the latest prompt on this repo.&gt;

## Change Log

| Date | Change | Worklog |
| ---- | ------ | ------- |
| 2026-06-17 | Initial binding (L7-001 sweep) | `worklogs/L7-001-intent-boundary-curation-2026-06-17.json` |
