...
out_of_scope:
  - "Pure-math VRAM / optimizer / Chinchilla primitives (lives in <REDACTED>/pheno-capacity substrate; hwLedger consumes it as a federated crate)"
  - "Org-wide audit scorecards, inventory of *all* Phenotype repos, pillar-level cross-repo health (lives in <REDACTED>/phenotype-org-audits)"
  - "LLM route / proxy plane (OmniRoute, Tokn, agentapi-plusplus, cliproxyapi-plusplus per ADR-ECO-007)"
  - "Generic observability / OTel export (lives in <REDACTED>/PhenoObservability and <REDACTED>/pheno-tracing)"
  - "Federated fleet dispatch / mesh control plane (lives in <REDACTED>/thegent, <REDACTED>/PhenoCompose, <REDACTED>/phenotype-infra)"
  - "Browser / IDE automation (lives in <REDACTED>/PlayCua, phenotype-journeys)"
  - "Phenokits / templates / scaffold commons (lives in <REDACTED>/phenokits-commons) — hwLedger is a consumer, not a template library"
depends_on:
  - "<REDACTED>/pheno-capacity (math substrate, federated crate; ADR-035A extraction)"
...
- **Repo:** `<REDACTED>/hwLedger` (GitHub remote: `<REDACTED>/hwLedger`)
- **Local path (container):** `repos/hwLedger/` (i.e. `/Users/<REDACTED>/CodeProjects/Phenotype/repos/hwLedger`)
- **Owner:** `<REDACTED>`
...
| -------- | -------- | ------ |
| Pure-math VRAM estimation, per-attention-kind KV cache math, MoE expert count, batch policy, Chinchilla tokens, optimizer-state size | `<REDACTED>/pheno-capacity` (per [ADR-035A](../adrs/ADR-035A-hwledger-reclassification.md), L5-115) | Federated lib; `no_std`-compatible, language-agnostic substrate. hwLedger consumes it as a federated crate. The math was extracted from `HwLedger/crates/capacity`, `HwLedger/crates/vram`, `HwLedger/crates/model-fit` per the substrate extraction of 2026-06-19. |
| Org-wide inventory / pillar scorecards / cross-repo health | `<REDACTED>/phenotype-org-audits` | Cohort-wide audit layer (165-repo inventory, 28 pillars incl. P26/P27/P28 phenodag orchestration). The hwLedger contribution is *one* input to the cohort inventory, not the cohort inventory itself. |
| LLM gateway / route / API proxy | `<REDACTED>/OmniRoute` (route), `<REDACTED>/Tokn` (route), `<REDACTED>/agentapi-plusplus` (cli_proxy), `<REDACTED>/cliproxyapi-plusplus` (cli_proxy peer), `<REDACTED>/bifrost` (vendor) — per [ADR-ECO-007](../adrs/ADR-ECO-007-gateway-merge-superset.md) | Three-layer topology keeps inference engines (omlx/mlx-lm) and isolation runtimes (nanovms/PhenoCompose) outside the router plane. hwLedger is a *consumer* of route intelligence, not a router. |
| Distributed tracing / OTLP metrics export | `<REDACTED>/pheno-tracing` (planned integration per `docs/integrations/pheno-capacity.md`) | OTLP substrate is fleet-wide; hwLedger will wire `pheno-tracing` once the integration lands. |
| App config (TOML overlay, settings lifecycle, feature flags, schema validation) | `<REDACTED>/Configra` (ADR-031) | App-level config layer lives in Configra; hwLedger migration slated. |
| Federated fleet mesh / process isolation / VM/sandbox | `<REDACTED>/PhenoCompose` (NVMS + process-compose — **AFFIRM** per ADR-ECO-007), `<REDACTED>/thegent` (Python agent dispatch) | hwLedger is a *probe* of these; it does not host the agent runtime. |
...
| Phenotype templating / scaffold commons | `<REDACTED>/phenokits-commons` | hwLedger itself is a consumer of `phenokits-commons` clean-rust / hexagonal-rust scaffolds (P1 ingestion crate uses pheno-*-lib patterns); it is not a template library. |
| Browser / IDE automation / E2E journeys | `<REDACTED>/PlayCua`, `<REDACTED>/phenotype-journeys` | Rich-media journeys under `apps/cli-journeys/` are owned and produced *by* hwLedger; *consuming* the journey harness for other repos is via `phenotype-journeys`. |

## Boundary Crossings

| Crossing | Direction | Surface | Status |
| -------- | --------- | ------- | ------ |
| `pheno_capacity::CapacityModel` (math) | `pheno-capacity` → `hwLedger` | Cargo crate dep (`<REDACTED>/pheno-capacity`) | green — active since 2026-06-18 (L5-105); Streamlit Planner / WhatIf consumption slated Phase 2 per `docs/integrations/cost-model-migration.md` |
...
| `HwLedger/crates/capacity`, `vram`, `model-fit` source artifacts | historical → `<REDACTED>/pheno-capacity` | git history (ADR-035A extraction, 2026-06-19) | green — superseded; see `registry/disposition-index.json` `lib-pheno-capacity` row |
...
| --- | --- | --- |
| Org-wide audit hub (canonical-home, no absorption) | [`phenotype-org-audits.md`](./phenotype-org-audits.md) | Same AFFIRM (self-as-canonical) shape: lives at `<REDACTED>/phenotype-org-audits`, owns cohort audit content, never absorbs elsewhere. hwLedger contributes its inventory to this cohort. |
...
- **No archive on GitHub.** Repo remains `<REDACTED>/hwLedger`, branch `main`. `git push` is **not** to be issued by this audit agent (parallel pushers may run).
...
| Top-level `Cargo.toml` | does not exist at root; only hit by `find -maxdepth 4 -name Cargo.toml` was inside `sidecars/omlx-fork/perf-core/` (a submodule of an already-extracted sibling repo `<REDACTED>/phenotype-omlx`) |
...
> Pure-math VRAM, KV cache, MoE expert count, batch policy, Chinchilla tokens, optimizer-state size — extracted from `HwLedger/crates/{capacity,vram,model-fit}` → `<REDACTED>/pheno-capacity` (L5-115, 2026-06-19).

The "code" the task brief presumed to absorb was **already extracted to `<REDACTED>/pheno-capacity`** three weeks before this task arrived. What remains in `repos/hwLedger/` is: README.md, applications (`apps/macos/` `.dmg`, `apps/streamlit/`, `apps/landing/`, `apps/build/`), docs site (`docs-site/.vitepress/dist/`), tools (`tools/`), CI (`/.github/`), the `sidecars/omlx-fork/` submodule (whose canonical home is `<REDACTED>/phenotype-omlx`), and root-level governance markdown (this boundary doc's dependencies).

**Submodule collision:** `hwLedger/.gitmodules` points `sidecars/omlx-fork` at `/Users/<REDACTED>/CodeProjects/Phenotype/repos/phenotype-omlx`, which is **already a standalone Phenotype-org repo at registry id `repo-phenotype-omlx`** (and which the registry this week marked `ARCHIVE_ONLY` per `559982e80`). Copying `sidecars/omlx-fork/` into `phenotype-infra` would duplicate an entity the org already absorbed.
...
- **B** — Force `ARCHIVE_ONLY` failsafe as literal text: skip copy/commit, run `gh repo archive <REDACTED>/hwLedger -y`, append this conflict log. **Rejected** because boundary doc line ~148 explicitly forbids archive; the failsafe text contradicts the boundary doc it was meant to author.
...
- `repos/hwLedger/` filesystem, git worktree, `<REDACTED>/hwLedger` GitHub remote all left untouched.
